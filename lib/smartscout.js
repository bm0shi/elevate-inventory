...
// opt.bbSum: the Buy Box % the third-party sellers with a file hold on this
// listing, added up. Amazon can't hold more than they leave, so it caps
// Amazon's share — the in-stock estimate put Amazon at ~95% on Tea Tree
// Lemon Sage while two sellers alone held most of the Buy Box, shrinking
// the pie to 882/mo against their 1,356/mo.
// opt.sellersSeen: third-party sellers known on the listing (seller files,
// plus us if we carry it) — the split is never fewer ways than that.
function pieFor(ss, k, opt) {
  ...
  if (opt.bbSum > 0 && amazonPct > 100 - opt.bbSum) { amazonPct = Math.max(0, 100 - opt.bbSum); amazonSrc = 'sellers'; }
  // FBA Sellers counts Amazon too when Amazon sells the listing.
  const amazonOn = amazonPct > 0 || (ss.amazonInStock || 0) > 0;
  const sellers3P = Math.max(1, (ss.fbaSellers != null ? ss.fbaSellers : 1) - (amazonOn && ss.fbaSellers ? 1 : 0), opt.sellersSeen || 0);
...
function sellerAbbr(name, isUs) {
  ...
  // The SD School storefront's name reads as "SOA" by initials; the owner calls it SD.
  return INITIALS_ABBR[initials] || initials;
}
