...
//           Amazon is in stock.
//   seller: the same kind of export for one seller (a competitor, or us).
...
const TEXT_FIELDS = new Set(['asin', 'title', 'brand', 'parentAsin', 'subcategory', 'refreshed']);
...
// Returns { columns, mapped: {field: header}, rows: [...], skipped }.
...
function parseSmartScout(text) {
  ...
  for (const r of all.slice(hi + 1)) {
    ...
    // Buy Box share may come as 0..1 or 0..100; store 0..100.
    if (o.buyBoxPct != null && o.buyBoxPct <= 1) o.buyBoxPct = o.buyBoxPct * 100;
    rows.push(o);
  }
  return { columns: header, mapped, rows, skipped };
}
...
module.exports = { parseCsv, parseSmartScout, pieFor, mergeRows, AMAZON_WIN_WHEN_IN_STOCK };
