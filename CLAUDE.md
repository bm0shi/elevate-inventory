...
| `lib/*.js` | Pure helpers moved out of `server.js` so they can be tested: `demand` (our share of a listing's sales from the Buy Box split, used by Send Next and Suggested Orders), `invoice-parse` (Cosmoprof/Xstore parsing, `findInvoiceDate`), `settlement-parse` (settlement flat files, `isPassThroughTax`, `INBOUND_FEE_PATTERNS`), `homebase` (timesheet CSV), `costs` (`blendCosts`), `matching` (invoice-line → product suggestions), `codes` (`normCode`, rack locations, `badQty`). Also `weekly-email` (Sunday summary email). No database access in the parsers. |
